...
def get_text_messages_by_id_ch_memoryEfficient(id_channel, db_name='Telegram_test'):
    with MongoClient(uri) as client:
        db = client[db_name]
        fs = gridfs.GridFS(db)
        grid_out = fs.get(id_channel)

        # Read the file in chunks to avoid MemoryError
        buffer = BytesIO()
        ...
        buffer.seek(0)  # Reset buffer to beginning before unpickling
...
if __name__ == '__main__':
    import_channels_to_mongoDB('Telegram_105_106')
    print(get_channel_ids("Telegram_105_106"))
